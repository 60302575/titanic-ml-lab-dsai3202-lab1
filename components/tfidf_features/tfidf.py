...
def main():
    ...
    deploy_df = pd.read_parquet(args.deploy)
    ...
    deploy_tfidf = vectorizer.transform(deploy_df["reviewText"].fillna(""))
    ...
    for split_df, matrix, out_path in [
        ...
        (deploy_df, deploy_tfidf, args.deploy_out),
    ]:
    ...
    print(f"TF-IDF done. Features: {len(feature_names)}")
